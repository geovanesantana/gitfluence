    ...
    <Layout>
      <div className='max-w-3xl px-5 xl:px-0'>
        <div className='mx-auto mb-5 flex max-w-fit translate-y-[-1rem] animate-fade-in items-center justify-center space-x-2 overflow-hidden rounded-full border border-light/20 bg-radial-gradient px-5 py-1 opacity-0'>
          <p className='text-sm font-semibold text-gray'>Fast & Easy Git Command Generator</p>
        </div>
        ...
        <p className='mt-6 translate-y-[-1rem] animate-fade-in text-center text-gray opacity-0 [--animation-delay:200ms] md:text-xl'>
          <Balancer>
            The AI-driven solution that helps you quickly find the right command. Get started with Git Command Generator
            today and save time.
          </Balancer>
        </p>
        ...
        {steps.map(({ icon, title, description }, index) => (
          <div
            ...
            className={classNames(
              ...
              `translate-y-[-1rem] animate-fade-in opacity-0 [--animation-delay:${index + 1}00ms]`
            )}
          >
            ...
            <div className='font-base prose-base max-w-xs leading-normal text-gray'>{description}</div>
          </div>
        ))}
      </div>
    </Layout>
...
const steps = [
  ...
  },
  {
    ...
    description: 'Now you can copy it and paste it into your terminal or command line interface.',
  },
];
